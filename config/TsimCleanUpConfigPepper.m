function config = TsimCleanUpConfigPepper(config)
% TSIMCLEANUPCONFIGPEPPER Test Userconfiguraton for not existing Parameters,
% MinSim and EasySpinIncompatibilities. If there were not all 
% minsimparameters in StandardSimulationParameter after Clean Up there are 
%
% Usage
%   config = TsimCleanUpConfigPepper(config);
%
%   config - struct
...
AllPoss = ParameterNames(userpar & pepperpar);
